 ...
 * Menggunakan Firebase Firestore untuk penyimpanan cloud
 */
...
const Storage = {
  ...
  async getAllMembers() {
    return await this.getCollectionData(this.COLLECTIONS.MEMBERS);
  },
  ...
  async addMember(name, position = '') {
    ...
    const success = await this.setDocData(this.COLLECTIONS.MEMBERS, newMember.id, newMember);
    if (success) return newMember;
    throw new Error('Gagal menambahkan anggota');
  },

  async getMemberById(id) {
    return await this.getDocData(this.COLLECTIONS.MEMBERS, id);
  },

  async deleteMember(id) {
    return await this.deleteDocData(this.COLLECTIONS.MEMBERS, id);
  },
  ...
  async exportData() {
    const [members, accessCodes, attendance] = await Promise.all([
      this.getAllMembers(),
    ...
    try {
      if (data.members) {
        await Promise.all(data.members.map(member =>
          this.setDocData(this.COLLECTIONS.MEMBERS, member.id, member)
        ));
      }
      if (data.accessCodes) {
  ...
  async clearAllData() {
    console.warn('Clear all data not implemented for Firestore. Gunakan Firebase Console untuk membersihkan data.');
    return false;
  },

  async getAllOsisStructure() {
    return await this.getCollectionData(this.COLLECTIONS.OSIS_STRUCTURE);
  },
  ...
  async addOsisPosition(position, name, level = 'member', parentLevel = null, photo = null) {
    ...
    const success = await this.setDocData(this.COLLECTIONS.OSIS_STRUCTURE, newPosition.id, newPosition);
  ...
  async updateOsisPosition(id, position, name, level = 'member', parentLevel = null, photo = null) {
    const existing = await this.getDocData(this.COLLECTIONS.OSIS_STRUCTURE, id);
    ...
    const success = await this.setDocData(this.COLLECTIONS.OSIS_STRUCTURE, id, updated);
    return success ? updated : null;
  },

  async deleteOsisPosition(id) {
    return await this.deleteDocData(this.COLLECTIONS.OSIS_STRUCTURE, id);
  },
